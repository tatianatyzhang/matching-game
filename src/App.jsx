...
const GameBoard = () => {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", alignItems: "center", padding: "20px" }}>
      ...
      >
        {/* Syriac Words (Left Side) - 2 Columns */}
        <div
          style={{
            display: "grid",
        ...
        >
          {shuffledWords.map(
            (word) => !matchedPairs[word.id] && <DraggableWord key={word.id} word={word} />
          )}
        </div>

        {/* Images with English Words (Right Side) - 3 Columns */}
        <div
          style={{
            display: "grid",
        ...
        >
          {shuffledImages.map((image) => (
            <DroppableImage key={image.id} image={image} onDrop={handleDrop} matchedWord={matchedPairs[image.id]} />
          ))}
        </div>
      </div>
